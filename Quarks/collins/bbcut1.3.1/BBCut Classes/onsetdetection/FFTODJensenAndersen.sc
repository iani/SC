...
//4 feature FFT onset detector after Jensen/Andersen paper- N.M.Collins 26/12/03

//amplitude thresholding

//separate out SynthDef creation function 

FFTODJensenAndersen : OnsetDetector
{
...
*new
{
^super.new.initFFTODJensenAndersen;
}

initFFTODJensenAndersen
{
	//threshold= 0.4;
...
["threshold", 0.001,2.0,\exponential,0.001,0.4]
]++(["spectral centroid","high freq energy","high freq content","spectral flux"].collect({arg v; 
[v, 0.0,1.0,\lin,0.001,0.25]
}));
...
node=SynthDef(\fftod1,
{
 var source1, proc, test, trig, diff, trig2, test2, combine;
	...
	proc= PV_JensenAndersen.ar(FFT(buf.bufnum,source1), params[2],params[3],params[4],params[5],params[1],params[0]);
