...
OnlineMIDI2 {
	...
	//analysis results:
	var <pitches, <iois, <intervals, <latestonset=0.0; //straight data arrays
	...
	*new{
	...
	analyse {|length=3.0, step=1.0|
		...
		routine= {
			...
			inf.do {
				...
				//Post << [now, then, data.collect{|val| val[0]}] << nl;
				...
				window= data.copyRange(index, data.size); //may be empty
				
				//ANALYSE WINDOW
				this.analyseWindow(now);
	...
	analyseWindow {|now|	
		...
		if (lasttime.notNil) {latestonset = now-lasttime} {latestonset = latestonset+1.0;};
		...
		{
		//keyname, key leave as was! 
		# pitchmax, pitchmin, pitchmean, pitchvar = [0,0,0,0];
		# volumemax, volumemin, volumemean, volumevar = [0,0,0,0];
		# intervalmax, intervalmin, intervalmean, intervalvar = [0,0,0,0];
		});
