...
RLVirtualPlayer2 {
	...
	var <rlstates, <prediction, <topK, numK=10, <lastTopK2; //prediction is one actually used for prediction, topK are top K scoring 
	var topKindex=0, lastTopKindex=0, lastTopKindex2=0; //gives index in topK of best scoring
	var listOnsets, listOffsets;
	var lastcreated, lastcreated2, newcreated;
	...
	var <>epsilon= 0.1; 
	var <>eligibilities, <ememory=5, <>lambda=0.9; 
	
	*new{|fsize=0.5, wframes=8|
	
	^super.new.initRLVirtualPlayer2(fsize, wframes);
	}
	
	initRLVirtualPlayer2 {|fsize=0.5, wframes=4|
	...
	trainFromMIDIFile {|path|
		...
		lastcreated2= nil; //avoid overlap effects
	...
	trainLive {
		...
		lastcreated2= nil; //avoid overlap effects with last session
...
//[Main.elapsedTime,starttime, duration,num,vel]
		...
		newcreated= RLState3.new; //create, pass all data in
	...
	updatestates {
		...
		lastTopK2= topK; 
		lastTopKindex2 = topKindex;
		lastTopK= lastTopK2; 
		lastTopKindex = lastTopKindex2;
...
//		});
		...
		if(islearning,{
			...
			if((lastTopK.notNil) && (prediction.notNil) && (lastcreated.notNil),{
			...
			if(tmp,{
			lastcreated.action= lastcreated2; //newcreated; 
		...
		lastcreated2= newcreated; //intermediate step must be allowed
		lastcreated = lastcreated2; //newcreated; 
	...
	findtopK {|state, index|
		...
		[winning, bestvalue, directprediction, bestvalue2].postln;
	...
	compareprediction {|statenow, statethen, bestmatch, list|
		...
		reward= statethen.proximity(statenow); //closer they are, more stable; no effect of intervening action
		...
		score2= reward; //updatestate.action.proximity(statenow);
		...
		//full sarsa update for the best performing, otherwise DO NOTHING gamma=0
		...
		//should be score2 not 1-score2? also sum score statethen.proximity(lastcreated2) + lastcreated2.proximity(statenow);
		
		//test= lrmod*((gamma*score3) + (1.0-score2) - (updatestate.v));
		
		test= lrmod*((gamma*score3) + (score2) - (updatestate.v));
		
		valuesum= valuesum + test;
		...
		updatestate.touched.add([owntime- (updatestate.lasttouch), valuesum- (updatestate.lastvalue), if((lastTopKindex==j) && (actedonprediction),1,0), valuesum, owntime, updatestate.lastvalue, reward, updatestate.lastreward]);  //5,6,7 indices
		updatestate.lastreward=reward;
