...
// 11 04 17 some modifications by IZ: 
// cosmetic code changes in startruntask
// Function:spectrogram made to take arguments and return synth like Function:play
...
Spectrogram {
	...
	var <frombin, <tobin;
	var image, imgWidth, imgHeight, index = 0, <>intensity = 1, runtask;
	...
	*new { arg parent, bounds, bufSize = 1024, color, background, lowfreq=0, highfreq=inf;
	...
	createWindow {arg parent, boundsarg;
		var bounds;
		window = parent ? Window("Spectrogram",  (boundsarg ?? { Rect(200, 450, 600, 300) }) );
...
SpectrogramWindow : Spectrogram { 
	classvar <scopeOpen;
	var startbutt;
	var <>width = 548, <>height = 328; 
	
	*new { | width = 548, height = 328 |
		^super.new.width_(width).height_(height);
	}
	...
	createWindow {
		...
		scopeOpen = true;
		window = Window("Spectrogram",  Rect(200, 450, width + paramW, height));
...
+ Function {
	spectrogram { | target, outbus = 0, fadeTime = 0.02, addAction=\addToHead, args |
		var synth;
