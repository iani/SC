...
SynthDefs { /* : Model */
	...
	*initClass {
		...
		this.loadFolder("~/scwork/synthdefs/*");
...
//			Post << d.name << " * ";
	...
	*makeGui {
		...
		window = GUI.window.new("SynthDefs", Rect(280, 0, 465, 330).fromRight);
		...
		GUI.staticText.new(window, Rect(0, 0, 300, 20))
			.font_(Font("Helvetica", 11))
			.string_(
		"\"e\" = edit, \"r\" = reload \"t\" = test, \"s\" = add script from synthdef"
			).align_(\center)
			.stringColor_(Color.red);
		foldersview = GUI.listView.new(window, Rect(0,0,120, 300))
			.resize_(4)
			.font_(Font("Helvetica", 11));
		listview = GUI.listView.new(window, Rect(0,0,120, 300))
			.resize_(4)
			.font_(Font("Helvetica", 11));
		controlsView = GUI.textView.new(window, Rect(0,0, 215, 300))
			.font_(Font("Helvetica", 11))
	...
	*makeControlSpecs { | desc, ctls, size |
		...
		^String.streamContents { | stream |
			...
			ctls.do { | ctl, i |
				...
				if (spec.isKindOf(EnvSpec)) { // 081104
					stream << "\n" <<< ([ctl.name.asSymbol] ++ [0, 0, 'linear', 0, 0]);
