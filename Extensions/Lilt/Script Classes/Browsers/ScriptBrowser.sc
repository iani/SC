...
4 Editing the selected script on a GUI.textView.new
...
		if (window.notNil) {
		...
		window = GUI.window.new("Script Browser-Editor", Rect(0, 0, 900, 400).fromTop);
		...
		serverview = GUI.button.new(window, Rect(50, 3, 73, 20));
		serverview.states = [
...
//		sinkview
		savebutton = GUI.button.new(window, Rect(257, 3, 40, 20))
			.states_([["Save", Color.grey(0.4), Color.white],
				["Save", Color.red, Color.white]
				]).font_(Font("Helvetica-Bold", 10))
			.action_({ this.saveScript })
			.enabled_(false);
		folderlist = GUI.listView.new(window, Rect(3, 25, 145, 370))
			.resize_(4).font_(Font("Helvetica", 10));
		scriptlist = GUI.listView.new(window, Rect(151, 25, 145, 370))
			.resize_(4).font_(Font("Helvetica", 10));
		scriptview = GUI.textView.new(window, Rect(298, 3, 600, 394))
			.resize_(5)
...
//			^thisMethod.report(folderPaths[currentFolderName])
		...
		scriptPath = (scriptPath.dirname ++ "/" ++ name.asString ++ ".scd").postln;
