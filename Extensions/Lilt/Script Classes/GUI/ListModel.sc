...
//		thisMethod.report("list changed");
		if (selection.notNil) {
//			thisMethod.report("selection was not nil");			
			this.selectItem(selection);
		}{
//			thisMethod.report("selection WAS NIL");
		};
//		thisMethod.report("the selection in detail:", selection, list);
	...
	addListView { | listview |
		...
		adapter = { | who, what, index, data |
			...
			switch (what,
				...
				\list, {
					listview.items = data; listview.value = index; }
			)
		};
...
//		thisMethod.report("listview items are:", listview.items);
//		this.changed(\list); // 080515
		this.listChanged;
...
//		thisMethod.report(index, itemName, item);
