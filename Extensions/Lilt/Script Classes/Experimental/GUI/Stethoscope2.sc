/* iz Sunday; November 16, 2008: 12:26 PM
A debugged and improved version of Stethoscope. 
- Restarts scope synth when stopped with Cmd-. 
- Notifies dependants when window closes and when the scoping synth stops.
	...
	*tileBounds {
	...
	makeWindow { arg view;
		if(view.isNil) 
		{
			...
			window.front;
			window.onClose = { window = nil; this.free; this.changed(\closed); };
	...
	run {
		thisMethod.report(window, synth, synth.isPlaying);
		if( window.notNil and: { server.serverRunning } and: { synth.isPlaying.not }) {
			...
			synth.onEnd( {
				this.changed(\synthStopped);
//				synth.isPlaying = false;
				synth = nil;
//				thisMethod.report("window", window);
				{ this.run }.defer(0.2); // if not deferred, the synt starts twice! (why?)
			} );
		};
	}
	...
	free {
		buffer.free;
		thisMethod.report(synth, synth.isPlaying);
