...
080424: (re-) implementing linking of script outputs to kr parameters via map and set. Also reimplementing midi binding mechanism using a new version of midi-listener. 
...
//		Each association is: ctlname->linkedbus. See method start */
	...
	// codes for modifier keys on apple keyboard under mac os x: 
...
//			start: #{ Routine({ 5.do { "empty script".postln; 1.wait }}).play };
		...
		krInputBusses = ();
//		this addDependant: { | ... args | args.postln };
		this.initScript;
	}
	initScript {
		var key, action, spec;
...
//		this.close;		// (close and) update GUtIs 
		...
		hasBufferInput = hasAudioInput = hasAudioOutput = hasControlOutput = hasTriggerOutput = false;
...
//		snapshots.postln;
	...
	doWhenSessionLoaded { | action |
		session addDependant: { |  who, what  |
			if (what == \loaded) {
				action.(this);
				this removeDependant: thisFunction;
			}
	...
	*openFolderDialog {
	...
	start { | ... args | // args may be passed e.g. by MIDIResponder functions 
		...
		if (process.respondsTo(\onEnd)) {
			process.onEnd({
				this.stopped;
				process.removeServer(server); // this may not be needed! please check and remove!
			}, server);
	...
	started { | ... senders |
		// prevent process being nil if started from outside, 
		// or start function returns nil
		if (process.isNil) { process = Script };
		{ this.changed(\started, *senders); }.defer;
	}
	onStart { | action |
	...
	onSet { | ... symbolActionPairs |
		var controller;
		controller = Controller(this, Environment make: { | controlEnv |
			symbolActionPairs pairsDo: { | symbol, action |
				controlEnv[symbol] = { | script, msg, val |
					envir use: { action.(val) }
				};
			}
		});
		if (this.isRunning.not) { controller.remove };
		this onStart: { controller.add };
		this onStop: { controller.remove }
	}
	writerStarted {
...
//		thisMethod.report("temporarily disabled");
...
//			[this, "informed: ", output, "that it stopped"].postln;
	...
	addDependantAction { | action, message |
		...
		this addDependant: { |  who, what  |
			if (what == message) { envir use: { action.(who, what) } }
		};
	}
	makeGui {
		...
		gui = GUI.window.new(name.asString, this.guiBounds);
		...
		snapshotNameEditView = GUI.textField.new(snapshotPanel, Rect(0, 0,
			decorator.innerBounds.width, 20))
			.canReceiveDragHandler_({ snapshots.selection.notNil
				and: { SCView.currentDrag isKindOf: MIDIResponder }
			})
			.receiveDragHandler_({
				snapshots.list.at(snapshotListView.value).midiResponder = SCView.currentDrag;
			}); 
		snapshotListView = GUI.listView.new(snapshotPanel, Rect(0, 0,
...
//					thisMethod.report(this, char, mod, alt);
	...
	makeMainButton { | argPanel, argWindow |
		var button, adapter;
		button = GUI.button.new(argPanel, Rect(0, 0, 145, 20))
...
*/	setButtonStates { | button, window |
		...
		#backcolor, offcolor, oncolor  = case
			...
			{ hasTriggerOutput } { [Color.white, Color.black, Color(1, 0.2, 0.2)] }
	...
	makeLinksGui {
		...
		linksWindow = GUI.window.new("Links: " ++ name.asString,
			this.guiBounds.moveBy(120, -25).resize(300, 200)
		).onClose_({
			linksWindow = nil;
			this.removeDependant(dependant);
		});
		writersView = GUI.listView.new(linksWindow, Rect(5, 0, 143, 196));
		readersView = GUI.listView.new(linksWindow, Rect(152, 0, 143, 196));
		readers = ListModel(
			this.getReaders,
...
//		thisMethod.report("writers", writers.list, "readers", readers.list);
...
//		writers.onSelect(writersView, _.postln);
	...
	getWriters {
		...
		^(parameters.collect({ | p | p.getAllWriterParameters +++ [p] }) ? []).flatten(1);
	}
...
*/
	...
	// as of 080424 the map mechanism may be rewritten ...
	map { | param, index |
...
//		this.inspect; param.inspect;
	...
	unMap { | param |
		// remove mapping when no script is controlling param by kr any more: 
		this.map(param, -1);			// undo the mapping of the parameter first
		krInputBusses.removeAt(param);	// then remove the parameter from kr mappings		
	}
	...
	close {
		...
		this.getReaders do: this.removeReader(_); // need to debug this for kr readers
		parameters do: _.removeAllWriters;
//		parameters do: { | p | p.postln.removeAllWriters };
	...
	// ---------------------------- BUFFERS ----------------------------
	loadBuffer { | bufName = 'buffer', param = 'bufnum', warnIfNotFound = true,
	// TODO: move most of this inside BufParameter, using buffer_ method etc. 
		numFrames = 44100, numChans = 1, server |
		// set a BufParameter's value to use a specific buffer given by name.
		// If not present, load it, and register it with Samples
		this.doWhenInited({
			// must be done after init to get the unique name of the script given by session
			var buffer;
//			thisMethod.report("starting", bufName, param, warnIfNotFound, numFrames, numChans, server);
			server = server ? session.server;
			if ((buffer = Samples(server).at(bufName.asSymbol)).isNil) {
				if (warnIfNotFound) {
					^Warn(format("Buffer: %  not found. Please load it to the system first", bufName));
				}{
					this.loadBufferFromPath([bufName.asSymbol, numFrames, numChans].asCompileString, param)
				}
			}{
				this.set(param, buffer.bufnum);
//				thisMethod.report("-----------------------------------", parameterEnvir[param]);
//				parameterEnv9
				this.changed(param, buffer.bufnum);
//				parameterEnvir[param].buffer = buffer;
				this.updateBufferInfo(buffer, param);
//				parameterEnvir[param].update(this, 'testing');
			};
		});
	}
	loadBufferFromPath { | path, param |
		 Samples(session.server).loadIfNotPresent(path, { | buf |
//			thisMethod.report(path, param, buf);
			this.set(param, buf.bufnum);
			this.changed(param, buf.bufnum);				
/*			{ 10.do: {
			}}.fork;
*////			thisMethod.report("INSIDE BUFINFO UPDATE", path, param, buf);
//			this.updateBufferInfo(buf, param);
		});
//		thisMethod.report(path, param, "DONE!!!!!!!");
	}
	updateBufferInfo { | buffer, param | 
		// TODO: ?? updateBufferInfo belongs to BufParameter and is called by \
		// BufParameter:buffer_ ?
		var adapter;
		adapter = { | buffer |
//			thisMethod.report(buffer);
			this.set(param, buffer.bufnum);
			buffer.removeDependant(adapter);
			{ buffer.waitForBufInfo }.defer(0.5);
		};
		buffer.addDependant(adapter);
		if (buffer.server.serverRunning) { buffer.changed(\info) }
	}
	
	allocBuffer { | bufName = 'buffer', numFrames = 2048, numChannels = 1, server |
...
//			thisMethod.report(this, src, chan, key, val);
	...
	makeScriptConnectionsGui {
		...
		*/
		...
		scriptConnectionsWindow = GUI.window.new("Links: " ++ name.asString,
			this.guiBounds.moveBy(150, -50).resize(450, 200)
		).onClose_({
			scriptConnectionsWindow = nil;
			this.removeDependant(dependant);
		});
//		scriptsView = GUI.listView.new(scriptConnectionsWindow, Rect(5, 2, 143, 196));
		sourcesView = GUI.listView.new(scriptConnectionsWindow, Rect(152, 2, 143, 196));
		targetsView = GUI.listView.new(scriptConnectionsWindow, Rect(300, 2, 143, 196));
		targets = ListModel(
			this.getTargets,
			makeNames: { | l | l.list collect:
				{ | p | [p.script.name ++ ":" ++ p.name, p] }
...
//		^envir[\targets];
	...
	showTargets {  // to be replaced by makeTargetsGui
		...
		{ | list |
			...
			w = GUI.window.new(list.name, Rect(30, 30, 350, 450).fromTop);
...
//				remove ?? {#{ |l, i| l.list.remove(i); l.list }}, // action for removing an item
			...
			tv = GUI.listView.new(w, decorator.innerBounds.width_(168));
			...
			tl.addListView(tv);
			lv = GUI.listView.new(w, Rect(0, 0, decorator.maxRight, decorator.maxHeight));
...
//					remove ?? {#{ |l, i| l.list.remove(i); l.list }}, // action for removing an item
				...
				listView = GUI.listView.new(mainPanel, decorator.innerBounds.width_(200));
...
//				snippets.selectAt(0);
	...
	*makeGlobalsWindow {
		...
		globalScriptsWindow = GUI.window.new.new("Global Scripts", Rect(0, 420, 150, 150).fromTop);
		globalScriptsWindow.onClose = { globalScriptsWindow = nil };
		scriptList = GUI.listView.new(globalScriptsWindow, Rect(3, 3, 144, 144))
			.resize_(5)
...
//	all these methods use the prefix r } */
	...
	*atGlobal { | key |
	...
	takeSnapshot {
		...
		thisMethod.report(this, "snapshot stored" /*, snapshots.list */);
	}
...
//		thisMethod.report("the snapshot is:", snapshot);
		snapshot do: { | v, i | 
//		thisMethod.report(v, i, parameters, parameters[i]);
			parameters[i].setPreset(v)
		};
	}
	...
	snapshotNamed { | argName |
		// get snapshot by name. argName should be a Symbol // previously: String
		^snapshots.list detect: { | s | s.first === argName }
	}
	loadInitSnapshot {
		var init;
		init = this.snapshotNamed(\init);
		if (init.notNil) { this.loadSnapshot(init[1]) };
	}
	// ================= Saving in Sessions  ================= 
...
*/
	saveData { | file |
		// save script-specific data in session file
		var action;
	...
	loadData { | data |
		// load script-specific data from session file
		envir[\loadData].(envir, *data);
//		this.changed(\dataLoaded);
	}
	// ---------------------------- PRINTING ----------------------------
	storeArgs { ^[name] }
	...
	// 080823 ff
	// -------------------------- CONDUCTOR STUFF -------------------------
	// deprecated. Use Conductor:addScript instead
	conductor { | conductor_func, argName, x = 128, y = 64, w = 900, h = 160, stopOnClose = true |
		// On start, open a gui based on Conductor. On stop, close that gui. 
		argName = argName ?? { name ++ ":conductor" }; 
		envir[\conductor] = Conductor.make(conductor_func);
		if (stopOnClose) {
			envir[\conductor].gui.addDependant { | who, how | 
				if (how === \closed) { this.stopped }
			};
		};
		envir[\start] = { envir[\conductor].show1(argName.(this), x.(this), y.(this), w.(this), h.(this)) };
		envir[\stop] = { envir[\conductor].hide };
	}
	conductorGui { | conductor_func, argName, x = 128, y = 64, w = 900, h = 160, stopOnClose = true |
		// replace your gui with a conductor gui!
		// under development 080823 ff
		var conductor;
		conductor = Conductor.make(conductor_func);
		envir[\conductor] = conductor;
		conductor.action_({ "Hola!".postln }, { "dallapiccola".postln });
		guiFunc = {
//			GUI.window.new(name, Rect(x, y, w, h)).front;
			gui = conductor.show;
			thisMethod.report(conductor.gui.header, conductor.gui.keys);
//			gui.postln;
		};
	}
}
