...
TrOutput -> 	Trigger (function) output	white
} */

AbstractOutputParameter : Parameter {
	// common superclass for parameters with signal output (KrOutput, ArOutput) and with action output (TrOutput)
	var <output; 	// a LinkedBus with links to all my readers or an array of objects receiving my output
	init { | argAction |
	...
	// Following definitions catch a problem as of 080502 wherein calling these in a subclass 
	// that does not define them causes SuperCollider to crash on MacOS X. 
	// Received from my script when it starts. For TrOutput: do nothing
	writerStarted {}
	writerStopped {}
}

SignalOutputParameter : AbstractOutputParameter {
	// common superclass for parameters with signal output (KrOutput, ArOutput)
	// ================ LINKING ==============
	getReaderParameters {
...
ArOutput : SignalOutputParameter {
...
KrOutput : SignalOutputParameter {
	init { | argAction |
		// register yourself to know when script started, to map your readers
		super.init(argAction);
		script.addDependant({ | sender, message |
			if (message === \started) { this.getReaderParameters do: _.applyMap }
		});
	}
	setScriptAttributes {	// set flag for script label color
...
//	canSendOutputTo { | argReader | argReader.isKindOf(AbstractOutputParameter).not and: { argReader.isKindOf(Parameter)} }
}
