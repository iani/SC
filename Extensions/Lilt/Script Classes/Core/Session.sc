...
TODO: 080516: improve the gui scheme. Use a simpler and better list implementation, possibly based on Conductor Quark.  

TODO: (older) audio i/o channels (???), server options (?)

Note: Wavetables or other script-private data of any type are saved inside the session, using custom functions stored in the environment of a script under ~saveData function. ~loadData is used for loading the saved data from the session script. 
...
Session {
	...
	*initMIDI {
	...
	makeGroups { // create the groups that this session needs to run
		...
		if (server.serverRunning) { //???? 080515
		}
	}
	...
	*initAll {
		...
		all = ListWithGui(
			...
			makeGui: { | sessions |
				...
				window = GUI.window.new("Sessions", Rect(0, 0, 210, 400).fromTop.fromRight);
				this.changed(\windowOpened);
				window.view.decorator = decorator = FlowLayout(window.view.bounds, 3@3, 2@2);
				// menu with all sesssions
				menu = GUI.popUpMenu.new(window, Rect(0, 0, decorator.rest[0], 20))
				...
				decorator.nextLine;
				menu = GUI.popUpMenu.new(window, Rect(0, 0, 60, 20))
				...
				decorator.nextLine;
				scriptGroupView = GUI.listView.new(window, Rect(0, 0, decorator.rest[0], 105));
				...
				decorator.nextLine;
				scriptView = GUI.listView.new(window, Rect(0, 0, *decorator.rest));
...
//					thisMethod.report("onSelect1. Session:", session);
					...
					if (session.notNil) {
						...
						session.scriptGroups.onSelect(scriptGroupView, { | sGroup |
							...
							if (sGroup.notNil) {
								...
								sessions.selection.currentScriptGroup.updateNames;
							};
						});
						session.scriptGroups.updateNames; // scriptGroupView.items = session.scriptGroups
//						thisMethod.report("onSelect2. Session scriptGroups selection:", session.scriptGroups.selection,
//							"scriptgroupview items:", scriptGroupView.items
//						);
						// update the views index to reflect currently selected script group
						// this here is in need of improvement 080516
//						session.scriptGroups.selectAt(0);
//						scriptGroupView.value =
//							session.scriptGroups.indexOf(session.scriptGroups.selection) ? 0;
//						thisMethod.report("session: %, scriptGroups list: %", session.name, session.scriptGroups.list);
						session.scriptGroups.selectAt(0);
						if (session.scriptGroups.selection.notNil) {
							session.scriptGroups.selection.selectAt(0);
//							thisMethod.report("this should be the selected script:", session.scriptGroups.selection.selection,
//								"and the value of the corresponding script view is:", scriptView.value);
//							scriptView.value
						};
//						thisMethod.report("session scriptgroups selection - current script group" , session.scriptGroups.selection);
//						
//						thisMethod.report(session.scriptGroups.selection, session.scriptGroups);
					} {
...
//				scriptView.canReceiveDragHandler = { true };
	...
	//			var session, scriptGroupList, newSession;
	//			var window, menu, scriptGroupView, scriptView, decorator, adapter;
//				thisMethod.report("all-list", all.list, "session:", session, "scriptGroupList:", scriptGroupList,
//					"scriptGroupView items:", scriptGroupView.items
//				);
//				if (all.size > 0 and: { all.selection.notNil }) { all.selectItem(session.selection) } { all.selectAt(0) };
				if (all.selection.notNil) { all.selectItem(all.selection) } { all.selectAt(0) };
////				all.selectAt(0);
				window;
			},
			list: [],
...
*/			160, { // option space: toggle guis of all scripts in group
	...
	addScriptGroup { | argName = 'Scripts', scripts |
		...
		scriptGroups.addSelect(ListWithGui(
			...
			makeGui: { | listModel |
				...
				window = GUI.window.new(listModel.name,
					(listModel.bounds ?? {Rect(200,0,200,300).fromTop.fromRight})
				);
				window.view.decorator = decorator = FlowLayout(window.view.bounds, 3@3, 2@2);
				GUI.staticText.new(window, Rect(0, 0, decorator.rest[0], 23))
					.align_(\center)
					.string = "(" ++ this.name.asString ++ ")";
				decorator.nextLine;
				listview = GUI.listView.new(window, Rect(0,0,*decorator.rest));
	...
	*load { | folder_name, script_name |
		// load a session from a script found in a folder in ScriptBrowser
		// this both loads the session-making script and creates its session: 
		ScriptBrowser.at(folder_name, script_name).makeGui;
	}
	*loadPath { | path |
		// load a session from a path to a session script file
		Script.load(path).makeGui.start;
	}
	*openDialog {
		CocoaDialog.getPaths({ | paths |
			this.loadPath(paths.first);
		});
	}
	*save {
		this.doIfSelected { | session | session.save; }
	}
	save { | showMessageWindow = false |
		var file;
		if (path.isNil) {
			this.saveAs;
		}{
			...
			if (showMessageWindow) {
				DialogWindow("Save Succeeded", format("saved session: %", name))
					.autoCloseAfter(1);
			}
		}
	}
