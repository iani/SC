...
// {
	...
	var <trWriters;	// array of TrOutputs that write to me (080511)
	var <normalizedVal;	// added 081026 for setIfNear;
// }
	...
	*forScript { | script, specs |
	...
	// with user-provided actions. 
		action.(*args);
	// added 081026 for setIfNear: 
		normalizedVal = spec.unmap(args[0]);
//		thisMethod.report(normalizedVal);
		script.changed(name, *args);
	}
	setIfNear { | val |
		// 081026: simple way to create "sticky" sliders. set only if unmapped value is within 0.02
		if (abs(val - normalizedVal) < 0.02) {
//			[val, spec.unmap(val), spec.map(val)].postln;
			this.set(spec.map(val));
		}
	}
	jog { | d |
		// 081026: for jogging = moving by an interval d
		normalizedVal = normalizedVal + d;
		this.set(spec.map(normalizedVal));
	}
	...
	makeGui { | gui, adapterEnvir |
		...
		slider = GUI.slider.new(gui, Rect(0, 0, 140, 20));
...
*/
	...
	getWriterParameters { if (input.isNil) { ^[] } { ^input.getWriterParameters.flat } }
	getAllWriterParameters {
		// also include TrWriters
		^this.getWriterParameters ++ trWriters;
	}
	...
	and would not create cycles if linked */
		if (writer.isKindOf(TrOutput)) { ^true };
...
//			"making BusLink for readers".postln;
	...
	inputRate { ^\control }
	removeAllWriters { // called by this.close, sent by Script:close to remove all synth links
	...
	// ------------------------- mapping and/or setting bus indices to parameters 
	input_ { | argInput |
	...
	resetInputBusIndex { | argIndex |
		...
		// doing if (script.isRunning) { script.parameter.map(name, argIndex) }
		script.map(this, argIndex);
	}
	applyMap {
		var node;
		// if script running, apply registed mapping to parameter. 
		// Called by KrOutput writing to you, when its script starts
		if ((node = script.process).isKindOf(Node)) {
			node.map(name, script.krInputBusses[this])
		}
	}
	krInputRunning {	// called by Script:start to test whether a parameter should be mapped
	...
	unmuteInput {} // ArInput removes itself from MuteBus here! What about kr???????????
	// synth order: 
	...
	// =================== TR links ==================
	addTrWriter { | trOutput |
		trWriters = trWriters add: trOutput;
		script.changed(\writers);
	}
	removeTrWriter { | trOutput |
		trWriters.remove(trOutput);
		script.changed(\writers);
	}
	// =================== MIDI ==================
