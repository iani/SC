...
// }
	...
	*withAll { | items |
	...
	makeGui {
		...
		learnButton = GUI.button.new(window, Rect(0, 0, 129, 20))
		...
		GUI.staticText.new(window, Rect(0, 0, 85, 20)).string = "Auto-Progress:";
		GUI.button.new(window, Rect(0, 0, 30, 20)).states_([["yes"],["no"]])
			.action_ { | me |
				autoProgress = if (me.value > 0) { false } { true };
			};
		GUI.staticText.new(window, Rect(0, 0, 100, 20)).string = "Note-On/Off pairs:";
		noteOnOffButton = GUI.button.new(window, Rect(0, 0, 30, 20)).states_([["no"],["yes"]])
			.action_ { | me |
				MIDIListener.noteOnOffPairs = if (me.value > 0) { true } { false };
			};
		GUI.button.new(window, Rect(0, 0, 40, 20)).states_([["Save"]])
			.action_ { this.save };
		GUI.button.new(window, Rect(0, 0, 55, 20)).states_([["Save as"]])
			.action_ { this.saveDialog };
		GUI.button.new(window, Rect(0, 0, 40, 20)).states_([["Load..."]])
			.action_ { |me| me.focus(false); this.loadDialog({|new| new.makeGui; }); };
		GUI.button.new(window, Rect(0, 0, 55, 20)).states_([["Revert"]])
			.action_ { this.class.load(path, {|new| new.makeGui; window.close }); };
		GUI.button.new(window, Rect(0, 0, 40, 20)).states_([["Name"]])
			.action_ { this.nameDialog; };
		GUI.button.new(window, Rect(0, 0, 40, 20)).states_([["Clear"]])
			.action_ { this.clear };
		window.view.decorator.nextLine;
		items do: { | item, count |
			GUI.staticText.new(window, Rect(0, 0, 97, 20)).string = rows[count][0];
